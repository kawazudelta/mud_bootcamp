...
- [x] **Containers:** Chests and crates (`testadv.objects.TestAdvContainer`).
- [x] **Sittables:** Chairs and benches (`typeclasses.sittables.Sittable`).
- [x] **Consumables:** Potions and food (`testadv.objects.TestAdvConsumable`).
- [x] **Echoing Rooms:** Atmospheric rooms (`testadv.rooms.EchoingRoom`).
- [x] **Random Loot:** Spawning via tables (Manual via `testloot` or scripted).
...
### 3.4. Interactive Objects
*Objects with specific behaviors enabled by their Typeclass.*

**Containers (Chests/Crates):**
```ev
@create/drop Heavy Chest;chest_01:testadv.objects.TestAdvContainer
#
@desc Heavy Chest = A reinforced iron chest.
#
# Optional: Set capacity (default 10)
# @set Heavy Chest/capacity = 20
```

**Sittables (Chairs/Benches):**
```ev
@create/drop Wooden Stool;stool_01:typeclasses.sittables.Sittable
#
@desc Wooden Stool = A simple three-legged stool.
#
# Optional: Customize the sitting message
# @set Wooden Stool/msg_sitting_down = "You rest your weary legs on the stool."
```

**Consumables (Potions/Food):**
```ev
@create/drop Healing Potion;pot_heal:testadv.objects.TestAdvConsumable
#
@desc Healing Potion = A flask of red liquid.
#
@set Healing Potion/uses = 1
#
```

**Echoing Rooms (Atmosphere):**
*Must use `testadv.rooms.EchoingRoom` instead of `typeclasses.rooms.Room`.*
```ev
@create/drop The Whispering Gallery;whisper_gal:testadv.rooms.EchoingRoom
#
@set The Whispering Gallery/echoes = ["The wind howls.", "Dust settles nearby.", "You hear a faint scratch."]
#
@set The Whispering Gallery/echo_rate = 60
#
@set The Whispering Gallery/echo_chance = 0.5
#
# Note: Echoes start automatically when the room script initializes (usually on creation/reload).
```
