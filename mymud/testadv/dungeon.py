...
def room_generator(dungeon_branch, depth, coords):
    ...
    '''
    room_typeclass = TestAdvDungeonRoom

    # simple map of depth to name and desc of room
    name_depth_map = {
        1: ("Overgrown Intake", "Ferns clog the ventilation slats. The air smells of wet rust."),
        2: ("Green Hydraulic", "Piston housing cracked by roots. Fluid leaks in slow, heavy drops."),
        3: ("Orange Warning", "Amber light filters through moss. The floor plates groan underfoot."),
        4: ("The Wailing Gap", "A door hangs off one hinge. The wind singing through it sounds like a name."),
        5: ("Purple Arteries", "Thick cabling pulses with violet light. It beats like a slow heart."),
        6: ("Aviary of Wires", "Loose cables hang from the ceiling. They sway without wind."),
        7: ("Rust Bloom", "The walls are flaking iron. Spores of oxidation drift in the light."),
        8: ("Glitch Garden", "Stone bricks jitter in place. A fern flickers between existence and static."),
        9: ("The Hollow Thorax", "Ribs of white metal arch overhead. The space is vast and waiting."),
        10: ("Ghost Assembly", "A pristine machine arm waits for input. The dust here is undisturbed."),
    }
    key, desc = name_depth_map.get(depth, ("Dark Rooms", "It's dark in here."))
...
class TestAdvDungeonBranch(DefaultScript):
    '''
    ...
    '''
    # Determines how branching the dungeon will be
    ...
    last_updated = AttributeProperty(datetime.now(timezone.utc))
    ...
    def register_exit_traversed(self, exit):
        '''
        Tell the system the given exit was traversed. 
        This allows us to track how many unvisited paths we have,
        so as to not have it grow exponentially.
        '''
    ...
    def create_out_exit(self, location, exit_direction="north"):
        '''
        Create outgoing exit from a room.
        The target room is not yet created.
        '''
    ...
    def new_room(self, from_exit):
        ...
        Args:
            from_exit (Exit): The exit leading to the new room.
        '''
        self.last_updated = AttributeProperty(datetime.now(timezone.utc))
...
class TestAdvDungeonStartRoomExit(DefaultExit):
    '''
...
class TestAdvDungeonBranchDeleter(DefaultScript):
    '''
    Cleanup script. After some time without a new room, a dungeon branch will 'collapse', 
    forcing all players in it back to the start room.
    '''
    ...
    def at_repeat(self):
        ...
        max_allowed_date = datetime.now(timezone.utc) - max_dt

        for branch in TestAdvDungeonBranch.objects.all():
            if branch.last_updated < max_allowed_date:
                # branch is too old; tell it to clean up and delete itself
                branch.delete()


class TestAdvStartRoomResetter(DefaultScript):
    '''
    Simple ticker-script. Introduces a chance of the room's exits cycling every interval.
    '''

    def at_script_creation(self):
        self.key = "testadv_dungeon_startroom_resetter"
    ...
    def at_repeat(self):
        ...
        for exi in room.exits:
            if inherits_from(exi, TestAdvDungeonStartRoomExit) and random() < 0.5:
                exi.reset_exit()
...
class TestAdvDungeonStartRoom(TestAdvDungeonRoom):
    ...
    def at_object_creation(self):
        ...
        create.create_script(
            TestAdvStartRoomResetter, obj=self, interval=self.recycle_time, autostart=True
        )
        create.create_script(
            TestAdvDungeonBranchDeleter,
            obj=self,
